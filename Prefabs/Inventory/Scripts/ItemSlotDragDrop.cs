...
public class ItemSlotDragDrop : TextureRect
{
    ...
    {
        popup = (PackedScene)ResourceLoader.Load("res://Prefabs/Inventory/Scenes/Popup.tscn");
    ...
    public override Control _MakeCustomTooltip(string forText)
    {
        Panel TooltipPopup = (Panel)popup.Instance();
        Label text = TooltipPopup.GetNode<Label>("Label");
        if(IsInstanceValid(slot.itemStruct)){
            text.Text = slot.itemStruct.description;
        } else {
            text.Text = "";
        }
